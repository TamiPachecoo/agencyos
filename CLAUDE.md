...
- **Technical Health** (`pages/health.html`, `js/health.js`, `css/health.css`): the "Critical Technical Issues" KPI and per-project "Technical health" row on the main dashboard were shipped as explicit "not tracked yet" placeholders (see dashboard commit) — this is the system that fills them in, once something actually writes to it. Two tables (`supabase/technical_health.sql`): `technical_health_reports` (one row per check — `checked_at`, `overall_status`, `summary`, `projects_checked`, `sources_checked`, `raw_report` jsonb) and `technical_health_findings` (`report_id`, `project_id` nullable + `project_name` text fallback so a finding survives a deleted project, `severity`, `category`, `title`, `details`, `likely_cause`, `recommended_action`, `status` — `new → investigating → fix_in_progress → resolved`, `source_refs` jsonb). Both RLS-restricted to `authenticated`, same pattern as `impact_workflows`/`impact_measurements` — anon gets nothing. The page itself is auth-gated the same way as Impact (`js/authState.js`, sign-in prompt when signed out).
  - Nothing writes to these tables yet — no cron job, no scheduled agent. `docs/WEEKLY_AGENT_PROMPT.txt` is the spec for one (checks GitHub/Vercel/Supabase/etc. per project, upserts a report + findings, never invents a check it can't actually run). Until that's wired up (e.g. via `/schedule`), the page just shows "No health check recorded yet."
  - The page lets findings be marked `investigating`/`fix_in_progress`/`resolved` by hand from a detail dialog (click any attention card or report row) — the weekly agent is only expected to insert new findings and preserve unresolved ones, not resolve them itself, so resolution is a manual action.
  - Wiring the dashboard's existing placeholders (`js/dashboard.js` — `computeAttentionItems`, `computeHealthDistribution`, the "Critical Technical Issues" KPI tile) to read from these tables was left undone on purpose, to keep this change scoped to the Health page itself — see the comments already in `dashboard.js` marking exactly where that seam is.
