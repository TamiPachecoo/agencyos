...
- **Project Questionnaires** (file-based): Projects can host custom questionnaire files (created by Claude Design or other tools) that clients fill out. Uploaded to project via project detail dialog, stored in Supabase Storage bucket `questionnaires`, tracked in `project_questionnaires` table. Each questionnaire gets a unique share token; clients open `pages/questionnaire.html?token=<token>` — a public form page that loads the file, renders it (in iframe for HTML files, embedded for PDF/DOCX), and captures form submissions. Responses saved to `questionnaire_responses` table; clients can submit once per questionnaire. Same token-based auth as Discovery Workbook (see security trade-off above). Two tables: `project_questionnaires` (id, project_id, file_name, file_path, share_token, created_at) and `questionnaire_responses` (id, questionnaire_id, response_data (jsonb), submitted_at).
